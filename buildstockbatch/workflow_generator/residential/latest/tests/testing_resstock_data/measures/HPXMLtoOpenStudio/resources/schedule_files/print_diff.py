...
def get_git_tracked_csv_files():
    ...
    result = subprocess.run(
        ["git", "ls-files", "*.csv"], capture_output=True, text=True, cwd=os.path.dirname(os.path.abspath(__file__))
    )
...
def compare_csv_files(file_path, branch=None, plot_profiles=False, output_dir=None):
    ...
    git_root = subprocess.run(
        ["git", "rev-parse", "--show-toplevel"], capture_output=True, text=True, cwd=script_dir
    ).stdout.strip()
                    ...
                    for idx in sample_indices:
                        if idx < len(df_git) and idx < len(df_current):
                            sample_changes.append(
                                f"row {idx}: '{df_git.iloc[idx][col]}' -> '{df_current.iloc[idx][col]}'"
                            )
            ...
            if plot_profiles and "daily average profile" in change:
                ...
                if avg_profile_git is not None and avg_profile_current is not None:
                    plot_path = plot_daily_profiles(
                        avg_profile_git, avg_profile_current, f"{os.path.basename(file_path)}: {col}", output_dir
                    )
                    if plot_path:
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="profile_plots", help="Directory to save plots (default: profile_plots)"
    )
